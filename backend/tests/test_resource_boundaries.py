...
def test_health_is_never_gated_and_stays_synchronous_and_trivial():
    ...
    # body 必須就是一行 return 模組常數。斷言「沒有函式呼叫」而不是比對整串字面值，
    # 這樣加一個常數欄位（例如 revision）不會誤報，但塞進任何 I/O 就會紅。
    statements = [line.strip() for line in body.strip().splitlines()[1:] if line.strip()]
    assert len(statements) == 1, f"/health 必須只有一行 return，實際：{statements}"
    assert statements[0].startswith('return {"ok": True'), statements[0]
    assert "(" not in statements[0], f"/health 回應不得有任何函式呼叫：{statements[0]}"
