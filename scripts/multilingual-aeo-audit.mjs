...
if (indexingConfig.INDEX_CONVERGENCE === true) {
  ...
  const sitemap = readFileSync(join(publicDir, 'sitemap-es.xml'), 'utf8');
  ...
  console.log(JSON.stringify({ status: 'PASS', mode: 'index-convergence', locale: 'es', noindexPages: files.length, sitemapUrlEntries: 0 }, null, 2));
  process.exit(0);
}
