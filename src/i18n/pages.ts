...
export const homePrivacy = {
  zh: {
    heading: '隱私與本機處理說明',
    text: 'FunnyTools 免費線上工具箱優先提供可在瀏覽器本機執行的工具。文字、名單與多數單檔圖片／PDF 工具通常不需要上傳；批次轉換、PDF 轉檔與 PDF 壓縮等需要網路服務的工具會在工具頁明確標示。',
    linkText: '查看完整隱私權政策',
  },
  en: {
    heading: 'Privacy & local processing',
    text: 'FunnyTools prioritizes tools that run in your browser. Text, lists, numbers, and many single-file image/PDF tools usually stay local; batch conversion, PDF conversion, and PDF compression pages identify their network processing clearly.',
...
export const homeFaq = {
  zh: [
    ...
    {
      q: 'PDF 工具會上傳檔案嗎？',
      a: '視工具而定：合併、拆分等整理工具在瀏覽器本機處理；PDF 轉 Word、表格轉 Excel 與 PDF 壓縮會把檔案暫時送到 FunnyTools Conversion API。請以各工具頁的隱私說明為準，並避免處理高度敏感資料。',
    },
    {
      q: '有哪些適合老師或學生的工具？',
  ...
  zh: {
    ...
    privacy: {
      ...
      body: [
        ...
        "本站是以瀏覽器本機工具為主的靜態網站。你在本機工具中輸入的文字、數字、名單與設定通常不會傳送到本站伺服器；需要網路服務的批次轉換與 PDF Conversion API 工具會在工具頁說明資料流。不要求註冊，也不建立會員帳號。",
        "本站預設不販售、出租或交換你在工具中輸入的內容，也不會把本機處理工具中的文字、名單、圖片或 PDF 內容提供給廣告商。若未來新增需要伺服器或第三方 API 的功能，會在該工具頁另外標示資料處理方式。",
        "本機處理範圍：文字處理、隨機抽選、計時、薪資與貸款估算、單檔圖片處理、PDF 整理、製圖與統計計算等工具，設計上以瀏覽器本機計算為主；批次轉換、PDF 轉檔與 PDF 壓縮會在工具頁標示 Conversion API 資料流。",
        "本機儲存：部分工具會使用瀏覽器的 localStorage 儲存你的偏好設定（例如番茄鐘的時間長度或介面選項），這些資料只留在你的裝置上，你可以隨時透過清除瀏覽器資料移除。",
        "電子報訂閱：若你自願訂閱電子報，本站會收集你的 email，透過 Brevo（Sendinblue）服務儲存與寄送新工具通知與站內更新。我們只用這個 email 寄送你訂閱的內容，不會販售或提供給第三方廣告商；退訂請依信件提供的方式（如有）、直接回覆或聯絡本站。",
        "檔案寄送功能：如果你在工具中選擇「把檔案寄到信箱」，本站會額外收集你的 email 與所使用的工具名稱，同樣透過 Brevo 寄送，並可能傳送符合限制的產生檔案。你選擇寄送的結果檔案只用於該次寄送；如需停止寄送或要求刪除已留存的 email 資料，可依信件提供的方式（如有）、直接回覆或來信聯絡本站。",
        ...
        "個別服務退出方式：若只想停用 Google Analytics，可安裝 Google 官方提供的分析退出擴充功能；若想停止收到電子報或檔案寄送信件，請依 Brevo 信件提供的退訂方式（如有）、直接回覆或來信告知，我們會依可用流程處理該 email。",
  ...
  en: {
    ...
    privacy: {
      ...
      body: [
        ...
        "The Site is a browser-first static site. Text, numbers, lists, and settings entered into local tools generally stay in your browser; batch conversion and PDF Conversion API tools identify their network data flow on the tool page. No account or registration is required.",
        ...
        "Local-processing scope: Text tools, random pickers, timers, salary and loan estimates, single-file image tools, PDF organizers, drawing tools, and statistics calculators are designed around browser-side processing. Batch conversion and PDF conversion/compression pages identify their network service.",
        ...
        "Newsletter subscriptions: If you voluntarily subscribe to the newsletter, the Site collects your email address and delivers new-tool announcements and site updates through Brevo (Sendinblue). We use that address only for the content you subscribed to; it is never sold or shared with third-party advertisers. To opt out, use any method provided in the message (if available), reply directly, or contact the Site.",
        "File-delivery feature: If you choose \"send the file to my inbox\" in a tool, the Site additionally collects your email address and the name of the tool used, and may send the generated file when it fits the delivery limits. A result file sent this way is used for that delivery; to stop delivery or request deletion of any retained email data, use any method provided in the message (if available), reply directly, or contact the Site.",
        ...
        "Opting out of specific services: To limit Google Analytics specifically, Google's own Analytics opt-out browser add-on is available directly from Google; to stop newsletter or file-delivery emails, use any opt-out method provided in a Brevo message (if available), reply directly, or contact us and we will process the request through the available list workflow.",
    ...
    aboutTools: {
      ...
      body: [
        "Most FunnyTools tools are static browser tools. After you open a tool page, form inputs, buttons, formulas, and result displays usually run inside your current browser tab; the tool page identifies any required server round-trip.",
        "Local-processing tools include word counters, case converters, text cleanup tools, random pickers, timers, salary and loan calculators, single-file image tools, PDF merge and split tools, statistical tests and percentile calculators, sketching, and flowchart tools. Batch image conversion, Image to DXF, PDF conversion, and PDF compression are separate Conversion API workflows.",
        ...
        "Many image and PDF organizers read files in your browser and generate downloadable results locally. Conversion API tools upload the input for server processing, and their pages explain the relevant limits before use.",
