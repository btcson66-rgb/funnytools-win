...
export default {
  zh: {
    ...
    contentSections: [
      ...
      },
      {
        ...
        paragraphs: [
          ...
          "彙整報告時，建議依封面、目錄、各部門章節、附錄的閱讀順序排列。每個部門可以保留自己的原始 PDF，最後只輸出一份交付版；若其中一份要抽掉多餘頁面，可先用頁面下方的 PDF 拆分工具連結取出需要的範圍，再回來合併。",
      ...
      },
      {
        ...
        paragraphs: [
          "限制是：工具在瀏覽器記憶體中讀取並重建 PDF，沒有伺服器上傳上限，但裝置效能會決定可處理規模。為了維持一般桌機與手機的穩定性，建議單次總檔案大小約 40 MB 以下、合併後總頁數約 300 頁以內；這是操作建議，不是保證值或硬性限制。高解析度掃描檔即使頁數少，也可能使用大量記憶體。",
    ...
    notes: [
      '建議單次總檔案約 40 MB 以下、合併後約 300 頁以內；這是瀏覽器穩定性建議，不是上傳限制。',
    ...
    faq: [
      ...
      {
        q: "手機可以合併 PDF 嗎？",
        a: "可以，但手機記憶體通常較少。建議處理小批次，單次總量盡量低於約 40 MB、300 頁，合併後立即下載並檢查。"
      },
      {
        q: "合併會修改原始 PDF 嗎？",
