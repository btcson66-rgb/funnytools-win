...
export const toolContentEnhancements: Record<string, Record<Locale, Enhancement>> = {
  'random-name-picker': {
    zh: {
      contentSections: [
        {
          heading: '姓名抽籤工具適合誰使用',
          paragraphs: [
            '姓名抽籤適合需要從一份名單中快速、公平選出人選的情境，例如老師課堂點名、學生報告順序、活動抽獎、社團分工或會議角色分配。只要把名單整理成一行一個名字，就能在瀏覽器中直接抽出一位或多位。',
            '這個工具重點是日常與教學現場的公平抽選，不需要登入，也不需要把名單交給外部服務處理。若名單包含學生個資、會員資料或活動報名資料，建議只輸入必要識別資訊，並避免在公開投影畫面顯示完整個資。',
          ],
          items: [
            '老師課堂點名、抽答、上台報告順序',
            '活動主持人抽獎、抽隊伍代表或抽分享者',
        ...
        },
        {
          heading: '姓名抽籤使用步驟',
          paragraphs: [
            '先把姓名或項目貼到文字框中，每一行放一個候選項目。工具會自動忽略空白行，但不會自動合併同名或重複行；如果班上有同名學生，建議加上座號、組別或代碼，讓抽選結果可以辨識。',
            '按「抽 1 位」會抽出單一結果；如果要一次抽出多位，先輸入數量再按「抽多位」。預設啟用「抽出後從名單移除」，被抽中的名字會從文字框移除，下一輪就不會再次被抽到。',
          ],
          items: [
            '一行一個名字或項目，空白行會被忽略',
            '抽多位時，數量不能大於目前剩餘項目數',
            '需要不重複抽選時，保留「抽出後從名單移除」',
            '抽完後可用「複製結果」貼到訊息、簡報或紀錄表',
          ],
        },
        {
          heading: '公平抽選與使用限制',
      ...
      instructions: [
        '貼上姓名或項目，每一行放一個候選項目。',
        '如果名單有同名者，先加上座號、組別或代碼方便辨識。',
        '按「抽 1 位」抽出單人，或輸入數量後按「抽多位」。',
        '需要不重複抽選時，保留「抽出後從名單移除」；需要保留原名單時，先取消勾選。',
        '抽選完成後，用「複製結果」保存名單或貼到公告、簡報、會議紀錄中。',
      ],
      examples: [
        '老師把全班名單貼入，隨機抽一位學生回答問題或上台分享。',
        '班會或營隊活動一次抽出多位工作人員、隊長或得獎者。',
        '社群活動主持人整理留言名單後，抽出指定名額並複製結果公告。',
        '團隊會議從成員名單中抽出記錄者、主持人或簡報順序。',
      ],
      audience: [
        '需要公平點名、抽答、安排報告順序的老師與補習班講師。',
        '舉辦班級活動、社團活動、抽獎或破冰遊戲的主持人。',
        '需要隨機分配會議角色、值日工作或討論順序的團隊。',
      ],
      caseStudies: [
        {
          title: '課堂抽答與報告順序',
          description: '老師貼上班級名單，保留抽出後移除，連續抽出本週報告順序。若有同名學生，先在姓名後加座號，抽到時就能直接確認人選。',
        },
        {
          title: '校園活動抽獎',
          description: '活動主持人把有效報名名單整理成一行一筆，輸入要抽出的名額後一次抽多位，再用複製結果貼到公告稿。正式獎項仍可搭配人工核對報名資格。',
        },
        {
          title: '會議角色分配',
          description: '團隊每週把成員名單貼入工具，抽出會議記錄、時間控管與下一位簡報者；啟用移除後，下一個角色不會重複落在同一人身上。',
        },
      ],
      notes: [
        '空白行會被忽略；重複的非空白行會被視為多個候選項目。',
        '抽選邏輯適合日常公平抽籤，不適合用作法律上具拘束力的抽獎、公證或博弈流程。',
        '若要公開投影抽選過程，建議只顯示必要稱呼或代碼，避免揭露完整個資。',
      ],
      faq: [
        {
          q: '姓名抽籤名單會上傳嗎？',
        ...
        {
          q: '可以一次抽出多位嗎？',
          a: '可以。先在數量欄輸入要抽出的人數，再按「抽多位」。抽選數量必須小於或等於目前名單中的候選項目數。',
        },
        {
          q: '怎麼避免同一個名字重複抽中？',
          a: '保留預設勾選的「抽出後從名單移除」。被抽中的名字會從文字框移除，下一輪抽選就不會再出現。',
        },
        {
          q: '同名學生或同名參加者怎麼處理？',
          a: '建議在名字後加上座號、組別、編號或其他可辨識代碼，例如「王小明 12」與「王小明 27」，避免抽到後無法確認是哪一位。',
        },
        {
          q: '空白行或重複行會影響抽選嗎？',
          a: '空白行會被忽略；重複的非空白行會被視為多個候選項目。如果不希望某人機率變高，請先清理重複名單。',
        },
        {
          q: '可以用在正式抽獎或有獎活動嗎？',
          a: '可作為一般活動抽選輔助，但不是公證抽獎系統。若涉及法規、獎項爭議或正式資格審查，請保留名單來源並搭配人工覆核。',
        },
      ],
    },
    en: {},
