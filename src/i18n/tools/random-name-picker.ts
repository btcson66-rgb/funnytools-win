...
export default {
  zh: {
    ...
    contentSections: [
      ...
      {
        heading: "公平抽選的程式方法與數字例子",
        paragraphs: [
          "這個頁面沒有使用 Math.random()。程式以瀏覽器的 crypto.getRandomValues 取得 32 位元亂數，先排除不能被候選數量整除的尾端區間，再取餘數，所以不會因直接取模讓部分索引多拿到一個可能值。接著使用 Fisher–Yates 方法由名單尾端逐步交換位置，最後取洗牌後的前幾項。若瀏覽器沒有安全亂數來源，程式會顯示錯誤並停止，不會偷偷改用較弱的替代方案。",
          "例如名單有 32 個不重複座號，單次抽一位時，每一列在程式模型中的機會都是 1/32。一次抽 3 位會先完整洗牌再取前 3 列，因此同一輪不會抽到同一列兩次；每一列進入這 3 個位置之一的機會是 3/32。若把同一姓名貼了兩列，那就是兩個候選項目，該姓名的中選機會也會加倍，所以抽選前仍要人工清理名單。",
          "「抽多位」在同一次操作內一定是不重複抽列，即使關閉抽出後移除也一樣；這個選項只決定中選列是否從文字框刪除。關閉時，下一次按抽選會從完整名單重新開始，所以前一輪的人可以再次中選；開啟時，下一輪只從剩餘列抽。若要做到全班一輪每人一次，應保持移除並在名單用完後再貼回原始名單。",
        ],
      },
      {
        heading: "公平不只取決於亂數",
        paragraphs: [
          "對學生而言，可檢查的程序比一句「電腦隨機」更重要。投影前先公布候選總數，讓全班確認缺席、請假與本輪豁免者；抽出後立刻說明是否移除，並在需要追蹤參與次數時保存結果。安全亂數只能讓每一列的抽選機會一致，無法保證題目難度、等待時間或被抽到後的評量方式同樣公平。",
        ],
      },
      {
      ...
      {
        heading: "低風險課堂活動的另一種隨機方式",
        paragraphs: [
          "姓名抽選適合從既有名單選人；若規則是用點數決定題目難度、答題組別或活動步數，骰子會讓學生更容易看到每個結果的固定範圍。",
        ],
        link: {
          prefix: "可改用",
          label: "線上骰子產生器進行 d6 或 d20 課堂活動",
          href: "/tools/dice-roller/",
          suffix: "，但兩者都只適用於教學與日常低風險決策。",
        },
      },
    ],
    examples: [
    ...
    notes: [
      ...
      "動畫只是延後顯示並輪播候選名稱；最終結果在動畫開始前已由安全亂數完成抽選，不能把動畫停在哪個名字當成重新抽選依據。",
      "安全亂數能降低可預測性，但無法修正缺席者未刪除、同一人重複登錄或資格名單本身不公平等資料問題。",
    ],
    formula: {
      expression: '均勻索引 = crypto.getRandomValues 的 32 位元值經拒絕取樣後 mod n；名單再以 Fisher–Yates 洗牌',
      explanation: 'n 是當下候選列數。程式先捨棄大於等於 floor(2³²/n)×n 的亂數，再取餘數 0 到 n−1；這使每個索引對應相同數量的 32 位元輸入值。',
    },
    educationApplication: '用於班級抽答時，建議先說明一行就是一個抽選單位，公開確認缺席與暫不參與名單，再決定本輪是否抽出後移除。這能改善程序透明度，但不保證每堂課的發言次數立刻相同；短期隨機結果仍可能連續落在同一組或相鄰座號。',
    reportTip: '若活動需要留下紀錄，可註明候選名單版本、抽選時間、是否啟用抽出後移除，以及複製出的結果；不要只保留截圖中的動畫畫面。',
    sources: [
      {
        label: 'W3C Web Cryptography：getRandomValues',
        href: 'https://www.w3.org/TR/webcrypto/#Crypto-method-getRandomValues',
        note: '規格將此方法定義為產生密碼學強度的亂數值；本站再加上拒絕取樣與 Fisher–Yates 洗牌，具體行為以頁面程式碼為準。',
      },
    ],
    faq: [
      {
