...
export default {
  zh: {
    ...
    seoTitle: '成績平均計算器｜算成績平均、加權總分，附試算範例',
    seoDescription: '免費成績平均計算器，貼上多筆考試、作業或小考分數與權重，立即算出總分、簡單平均與加權平均，教師甄試、學期成績試算都好用。',
    ...
    contentSections: [
      ...
      {
        heading: '這個計算器實際怎麼處理分數與精度',
        paragraphs: [
          '程式會把每一列可解析為有限數值的分數納入簡單平均，空白或無效分數列不計；分數沒有被限制在 0 到 100，所以加分題、百分制以外量尺也能輸入。加權平均則只納入「分數有效且權重大於 0」的列，權重空白、0 或負數都不進入加權公式。若只有部分列填權重，畫面上的簡單平均與加權平均會使用不同資料列，解讀時要特別留意。',
          '加總與除法使用瀏覽器的完整數值精度，沒有先把每列乘積四捨五入；畫面、複製摘要與 CSV 統計列最後才以小數點後 2 位呈現。若學校規定每一項先取整數、學期總分再四捨五入，結果可能和本工具不同，應以正式評分規準指定的計算順序為準。',
        ],
      },
      {
        heading: '台灣段考與平時成績的數字範例',
        paragraphs: [
          '假設某生平時成績 88 分、第一次段考 76 分、第二次段考 84 分，課程規定權重依序為 30、30、40。加權總和為 88×30 + 76×30 + 84×40 = 8,280，除以權重總和 100 後是 82.80 分；同三筆分數的簡單平均則是 82.67 分。兩者不同不是工具算錯，而是後者把三項視為同等重要。',
          '若「缺考」尚未依校規決定要記 0、補考成績或不列入，請先不要用空白代替任何一種正式處置。空白列會被忽略，和明確輸入 0 分的結果不同。',
        ],
      },
      {
        heading: '從一般成績平均延伸到教甄加權試算',
        paragraphs: [
          '一般成績平均適合任意多列分數；若你要固定比較筆試、試教與口試三項，並檢查權重是否合計 100%，可改用專門的教甄情境工具。',
        ],
        link: {
          prefix: '接著使用',
          label: '教師甄試成績轉換模擬器檢查筆試、試教與口試權重',
          href: '/tools/teacher-exam-score-converter/',
          suffix: '，並以當年度簡章核對門檻與同分比序。',
        },
      },
    ],
    instructions: [
    ...
    notes: [
      ...
      '樣本很少時，平均容易被單次極高或極低分數拉動；若要描述全班分數分散程度，應同時查看標準差、分布圖與有效人數。',
      '本工具不知道各校的缺考、補考、加分、及格門檻或成績進位規則，輸入前必須先依正式辦法整理資料。',
    ],
    formula: {
      expression: '簡單平均 = Σxᵢ / n；加權平均 = Σ(wᵢxᵢ) / Σwᵢ',
      explanation: 'xᵢ 為每筆有效成績，n 為有效成績筆數；wᵢ 為正數權重。程式以所有有效分數計算簡單平均，但加權平均只使用同時具有有效分數與正數權重的列；運算完成後才將畫面結果四捨五入到小數點後 2 位。',
    },
    educationApplication: '可用於形成性評量、平時成績或學期成績的初步整理。平均數回答的是「這些有效項目的中心大約在哪裡」，不等同是否達成課綱或校訂及格標準；前者是資料摘要，後者是標準參照判定。若有缺考、補考、加分、最低門檻或特殊四捨五入規則，應先依正式評分規準處理。',
    ...
    sources: [
      {
        ...
        note: '公開列出算術平均與加權平均 Σ(wᵢxᵢ)／Σwᵢ 的定義；本頁公式依此慣例，輸入與顯示規則則以本站程式實作為準。',
      },
    ],
    faq: [
      {
