...
export const additionalSeoGuides: SeoGuide[] = [
  ...
  {
    id: '2d-cad-drawing-board-workflow-guide',
    locales: ['zh', 'en'],
    ...
    priority: 55,
    searchIntent: { zh: '使用者想在不安裝 CAD 軟體的情況下，畫出有格線、直角與可交付尺寸的簡單 2D 草圖，並知道 PNG 與 SVG 該怎麼選。', en: 'A user wants to make a simple 2D sketch with a grid, right-angle geometry, and a usable export without installing desktop CAD software.' },
    targetKeywords: [
      { zh: '2D CAD 線上繪圖', en: '2D CAD drawing online' },
      { zh: '線上 CAD 畫圖工具', en: 'online CAD drawing tool' },
      { zh: '2D CAD PNG SVG 匯出', en: '2D CAD PNG SVG export' },
    ],
    relatedToolIds: ['cad-2d', 'image-resizer', 'image-compressor'],
    relatedGuideIds: ['cad-grid-snap-orthogonal-guide', 'svg-vs-png-diagram-export', 'flowchart-design-checklist'],
    relatedWorkflowIds: [],
    summary: { zh: '把 2D 草圖拆成版面設定、幾何繪製、檢查與匯出四段，讓線上繪圖板適合快速示意、教學與溝通，而不誤當成完整工程 CAD。', en: 'Treat a browser CAD sketch as four stages—setup, geometry, inspection, and export—so it works for quick diagrams and teaching without pretending to replace full engineering CAD.' },
    problem: { zh: '很多人打開線上繪圖板就直接拉線，結果比例忽大忽小、直角不準、文字貼到邊界，或匯出後線條在簡報與列印中變得太細。先確定工具能做什麼，再按交付用途驗收，通常比事後補救快。', en: 'Starting with random lines often produces inconsistent scale, crooked corners, labels on the edge, or strokes that disappear in a slide or printout. Confirm the tool’s scope first, then inspect the export for its real destination.' },
    whoShouldUse: { zh: '適合教師示範幾何與平面配置、學生交作業草圖、產品或活動團隊畫簡單配置，以及需要快速分享示意圖的使用者。涉及精密製造、完整圖層、尺寸標註標準或工程簽核時，應改用合適的專業 CAD。', en: 'Useful for teachers, students, product or event teams, and anyone sharing a quick layout. Precision manufacturing, full layer control, standards-based dimensioning, or engineering sign-off still require dedicated CAD software.' },
    explanation: [
      { zh: '先設定畫布用途與比例。若只是課堂示意，先決定「一格代表多少」並在標題寫清楚；若要放進簡報，先用投影片實際寬度估算線條與文字大小。瀏覽器畫布的像素不是工程圖的實體單位，不能直接宣稱是毫米或英吋。', en: 'Start with the canvas purpose and a working scale. For a class sketch, state what one grid cell represents; for a slide, estimate stroke and label size against the real slide width. Canvas pixels are not engineering millimetres or inches by themselves.' },
      { zh: '打開格線與吸附後再畫主要輪廓。格線提供視覺參考，吸附則把端點靠近規則位置；兩者都不是測量儀器。先畫外框、中心線與主要開口，再補次要線段，能減少每一段各自漂移。', en: 'Turn on the grid and snap before drawing the main outline. The grid is a visual reference and snap moves endpoints toward regular positions; neither is a measurement instrument. Draw the outer boundary, centerline, and openings first, then add secondary edges.' },
      { zh: '需要水平或垂直邊時使用正交模式，不要靠眼睛把游標拉直。正交線適合房間平面、盒體、座位配置與簡單流程框；斜線或自由形狀則應關閉正交，避免工具替你改變原本想畫的角度。', en: 'Use orthogonal mode for horizontal and vertical edges instead of trying to straighten the pointer by eye. It suits room plans, boxes, seating layouts, and simple frames; turn it off for deliberate diagonals or free shapes.' },
      { zh: '把文字、箭頭與輪廓分開檢查。線條重疊時，先確認讀者要看的主路徑；標籤不要壓在交點或貼著畫布邊界。這類圖的目標是讓人快速理解空間關係，不是把每個像素塞滿。', en: 'Inspect labels, arrows, and outlines separately. When strokes overlap, preserve the reader’s main path and keep labels away from intersections and canvas edges. The goal is fast understanding of spatial relationships, not filling every pixel.' },
      { zh: '匯出前先做三個回讀：縮放到 100% 看細線，縮小到預計顯示尺寸看文字，最後用另一個檢視器開啟檔案。PNG 適合需要固定像素的簡報截圖或列印草稿；SVG 適合需要縮放而不想讓幾何線條變糊的網頁或設計流程，但接收端必須支援 SVG。', en: 'Before export, read the drawing three ways: inspect strokes at 100%, view it at the intended display size, and open the file in another viewer. PNG suits fixed-pixel slides and print drafts; SVG keeps geometric lines scalable for web or design workflows, provided the recipient supports SVG.' },
      { zh: '這個工具的邊界要寫在交付說明中。FunnyTools 2D CAD 繪圖板適合瀏覽器內的快速草圖與 PNG／SVG 輸出；它不會建立 DWG／DXF 工程資料庫，也不會替你做公差、圖層標準或製造檢核。清楚說明限制反而能避免收件人把示意圖當成施工依據。', en: 'State the tool’s boundary in the handoff. FunnyTools’ 2D CAD board is for quick browser sketches and PNG/SVG output; it does not create a DWG/DXF engineering database or perform tolerance, layer-standard, or manufacturing checks. Clear limits prevent a sketch from being mistaken for construction data.' },
      { zh: '若要交作業或開會，檔名加入主題、比例假設與日期，並保留一份未壓縮的 SVG 或原始草稿。之後若只剩一張模糊 PNG，很難判斷是繪製錯誤還是匯出尺寸不足。', en: 'For assignments or meetings, put the subject, scale assumption, and date in the filename, and keep an uncompressed SVG or source sketch. A lone blurry PNG makes it hard to tell whether the geometry or export size was at fault.' },
    ],
    steps: [
      { zh: '寫下圖的用途、讀者、畫布比例假設與不可漏掉的標籤。', en: 'Record the purpose, audience, scale assumption, and labels that must remain.' },
      { zh: '在 2D CAD 繪圖板開啟格線，依需要啟用吸附與正交線。', en: 'Open the 2D CAD board, enable the grid, and turn on snap or orthogonal mode as needed.' },
      { zh: '先畫外框、中心線與主要開口，再補細節和文字。', en: 'Draw the outer boundary, centerline, and main openings before details and labels.' },
      { zh: '關閉或開啟正交模式逐一完成直角與斜線，避免模式誤套。', en: 'Toggle orthogonal mode deliberately for right angles and diagonals so the wrong constraint is not applied.' },
      { zh: '以 100%、實際顯示尺寸與另一個檢視器回讀線條、文字、邊界和重疊。', en: 'Read back strokes, labels, boundaries, and overlaps at 100%, target display size, and in another viewer.' },
      { zh: '按用途選 PNG 或 SVG，加入日期與比例假設後下載，並保留來源版本。', en: 'Choose PNG or SVG for the destination, add the date and scale assumption, download, and keep the source version.' },
    ],
    example: { zh: '老師要示範教室座位配置，先把畫布定為「一格代表一排間距」，開啟格線與正交線，畫出講台、四列桌子和逃生通道，再把組別名稱放在不遮住線條的位置。課堂投影用 PNG，課後講義用 SVG；兩份都標示「示意圖，非實際尺寸」，避免學生誤把像素當成公分。', en: 'A teacher sketches a classroom seating plan. They state that one grid cell represents one row gap, enable grid and orthogonal lines, draw the desk rows and exit path, and place group labels away from strokes. They export PNG for projection and SVG for the handout, both marked “illustration, not to scale” so pixels are not mistaken for centimetres.' },
    commonMistakes: [
      { zh: '沒有寫比例假設，卻把畫布像素當成實體尺寸。', en: 'Leaving the scale assumption unstated and treating canvas pixels as physical units.' },
      { zh: '只靠目測畫直角，造成輪廓慢慢偏斜。', en: 'Drawing right angles by eye and letting the outline drift.' },
      { zh: '忘記切換正交模式，讓斜線被限制成水平或垂直。', en: 'Forgetting to toggle orthogonal mode and forcing an intended diagonal into a right angle.' },
      { zh: '只看放大預覽，沒有在投影片或手機的實際尺寸檢查文字。', en: 'Checking only a zoomed preview instead of the text at slide or phone size.' },
      { zh: '把 PNG 壓得太小，或把 SVG 交給不支援的系統。', en: 'Making the PNG too small or sending SVG to a system that cannot display it.' },
      { zh: '把快速示意圖當成有尺寸、公差與工程簽核的正式圖面。', en: 'Treating a quick sketch as a dimensioned, toleranced, and approved engineering drawing.' },
    ],
    faq: [
      { question: { zh: '線上 2D CAD 可以取代 AutoCAD 嗎？', en: 'Can a browser 2D CAD board replace AutoCAD?' }, answer: { zh: '不能一概而論。它適合快速平面草圖、教學與溝通；若需要 DWG／DXF、完整圖層、精密標註、協作權限或工程簽核，應使用符合規格的專業 CAD。', en: 'Not generally. It is useful for quick sketches, teaching, and communication; DWG/DXF, full layers, precision dimensions, permissions, and engineering approval require suitable professional CAD.' } },
      { question: { zh: '2D CAD 草圖要下載 PNG 還是 SVG？', en: 'Should a 2D CAD sketch be exported as PNG or SVG?' }, answer: { zh: '固定像素的投影、截圖或簡報通常用 PNG；需要縮放且接收端支援向量圖時用 SVG。無論哪種格式，都要在實際顯示尺寸檢查線條與文字。', en: 'Use PNG for fixed-pixel projection, screenshots, or slides; use SVG when scalable geometry is needed and the recipient supports vector files. Check strokes and labels at the real display size either way.' } },
      { question: { zh: '格線等於精確測量嗎？', en: 'Does the grid provide precise measurement?' }, answer: { zh: '不是。格線與吸附協助畫出一致的相對位置，但瀏覽器畫布沒有自動變成工程單位；要做正式尺寸仍需專業 CAD 或其他測量流程。', en: 'No. Grid and snap help with consistent relative placement, but a browser canvas does not become an engineering unit system. Use professional CAD or a measurement workflow for formal dimensions.' } },
      { question: { zh: '為什麼匯出的線條看起來太細？', en: 'Why do exported lines look too thin?' }, answer: { zh: '可能是把圖縮得太小、投影或列印解析度不同，或 SVG 檢視器套用不同縮放。先在預計尺寸回讀，再增加線寬或改用較大的 PNG 輸出。', en: 'The drawing may be displayed or printed smaller, rendered at a different resolution, or scaled differently by an SVG viewer. Read it at the target size, then increase stroke width or export a larger PNG.' } },
    ],
    cta: { zh: '打開 FunnyTools 2D CAD 繪圖板，先設定格線與正交模式，完成一張標示用途與比例假設的草圖，再按交付情境匯出 PNG 或 SVG。', en: 'Open FunnyTools’ 2D CAD drawing board, set the grid and orthogonal mode, make a sketch with its purpose and scale assumption, then export PNG or SVG for the real destination.' },
    updatedAt: '2026-09-03',
    noFaqSchema: true,
  },
];
