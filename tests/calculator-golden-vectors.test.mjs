 ...
 *
 * ── 本檔三種斷言的證據強度不同，不可混為一談 ──────────────────────────
 *   (A) 數學獨立向量：期望值來自教科書公式、標準定義或可手算案例，
 *       與 production 走不同路徑。**這才是正確性證明。**
 *   (B) 不變量測試：不宣稱某個數字對，只宣稱某個關係恆成立
 *       （例如攤還表期末餘額趨近 0、機率和為 1）。
 *   (C) 來源錨點（GV-000）：只是「轉錄沒有漂移」的結構守門，
 *       **不是**正確性證明——它比對的是 production 自己的字串。
 *       任何只由 (C) 支撐的結論都不能當成「已驗證正確」。
 *
 * 產品規格未定義的行為（例如百分比變化的負基準）不放進 (A)。
 * 那類項目寫成明確標示的特性化測試（characterization），
 * 用途是「行為若被改動要有人知道」，不是「目前行為是對的」。
 */
...
const SOURCE_ANCHORS = [
  ...
  ['PercentageCalculator', 'const change = ((b - a) / a) * 100;'],
...
// GV-506（顯示值對帳）已移除。
// 畫面上的「總收入 / 扣除合計 / 估算實領」各自 Math.round，所以三者相減不一定
// 剛好相等（20,000–80,000 每 100 元一階、四組常見費率的 2,436 種輸入中約 5%
// 會出現 1 元落差）。
//
// 這是**呈現層捨入 vs 底層計算捨入**的產品定義問題，不是計算錯誤：底層
// income − deductions 一直是對的。要讓畫面數字逐項相加完全相等，得決定
// 「顯示的實領以已捨入的兩者相減得出」，那會讓顯示值與真實 net 差到 1 元。
// 這個取捨要由產品決定，不由可靠度稽核決定。
// 待決事項：DECISION_NEEDED_NET_SALARY_DISPLAY_ROUNDING。
//
// 這裡不留斷言：在目前實作下，任何「顯示值自洽」的斷言都只能在測試內自行
// 重算一次顯示值，等於斷言測試自己的算術，是廢測試。
...
test('GV-705 [特性化] 百分比變化：負基準沿用已載明的 (B−A)÷A 規格', () => {
  // 這**不是**數學獨立向量，是特性化測試：釘住目前行為，改動時要有人知道。
  //
  // 中英文 FAQ 自 2026-06-19 起載明的公式就是 (B − A) ÷ A × 100，
  // production 也照這個實作。當 A 為負時，除以帶號的 A 會讓結果的符號、
  // 以及介面上的「增加／減少」方向字，與數值本身的變化方向相反：
  // −80 → −100 數值變小，但這個公式算出 +25%，方向字是「增加」。
  //
  // 產品從未定義負基準應該怎麼算。改成除以 |A| 是**新的產品定義**，
  // 不是修 bug——會同時讓已公開的 FAQ 公式失效，所以不在可靠度範圍內處理。
  // 待決事項：DECISION_NEEDED_NEGATIVE_BASE_PERCENT_CHANGE。
  const r = percentage('change', -80, -100);
  assert.equal(Number(r.value.toFixed(6)), 25,
    'GV-705 (−100 − (−80)) ÷ (−80) × 100 = +25，符合已載明的公式');
  assert.equal(r.word, 'increase',
    'GV-705 方向字直接取自 change 的正負；負基準時它與數值變化方向相反，這是已知且未決的語意問題');
});
