...
test('§4 [RED] backend tables_to_xlsx 必須中和 = 開頭的儲存格', () => {
  // 現況：backend/services/pdf_table.py:228 直接 ws.cell(r, c, str(value))。
  // openpyxl 的 Cell._bind_value 會把 len>1 且以 "=" 開頭的字串標成 data_type='f'，
  // 也就是寫成真正的 Excel 公式（實測見 output-safety-test-run.md）。
  // 惡意 PDF 表格、或直接 POST /api/pdf/export-tables 的 JSON，都能塞入
  // =HYPERLINK(...) 之類的公式。
  // 修法（本輪不做，只稽核）：寫入前對 "=" 開頭的字串加上 "'" 前綴，
  // 或改用 ws.cell(...).value 搭配明確的 data_type='s'。
  const backend = readSrc('backend/services/pdf_table.py');
  const writer = backend.slice(backend.indexOf('def tables_to_xlsx'), backend.indexOf('def pdf_tables_to_xlsx'));
  const neutralised = /startswith\("="\)|startswith\('='\)|escape_formula|_safe_cell|data_type\s*=\s*["']s["']/.test(writer);
  assert.ok(
    neutralised,
    'tables_to_xlsx 沒有任何公式中和步驟：ws.cell(r, c, str(value)) 會讓 "=" 開頭的儲存格'
      + ' 被 openpyxl 標成 data_type="f"，也就是寫成真正的 Excel 公式。',
  );
});
